...
class Agent:
    def __init__(
        ...
        if self._netting:
            ...
            self.broker = NettingBroker(broker, cost=self._paper_cost, order_vet=vet,
                                        catastrophe_stop_mult=cat_mult,
        ...
        self._filters = FilterConfig.from_dict(config.get("filters", default={}))
    ...
    async def fast_step(self) -> None:
        ...
        if self.shadow_broker is self.broker and self._account_is_paper():
            ...
            self.risk.update_book("shadow", await self.shadow_broker.account_equity())
        else:
        ...
        # Rebuild open exposure (symbol -> notional) so the portfolio caps actually
        # bind. Without this the exposure/position limits read an empty dict and
        # never trigger. Notional uses the live mark when available.
        try:
            exposure: dict[str, float] = {}
            for p in positions:
                px = p.entry_price or 0.0
                exposure[p.symbol] = exposure.get(p.symbol, 0.0) + abs((p.volume or 0.0) * px)
            self.risk.open_exposure = exposure
        except Exception as e:  # noqa: BLE001 — risk refresh must not kill the loop
            log.warning("Could not refresh open exposure: %s", e)
    ...
    def _apply_control(self) -> None:
        ...
        if "filters" in ctrl:
            from .filters import FilterConfig
            self._filters = FilterConfig.from_dict(ctrl.get("filters"))
    ...
    def _unrealized(self, p) -> float | None:
        """Mark-to-market running P/L for an open position from the latest view price."""
        mark = (self._last_view.get(p.symbol) or {}).get("price")
    ...
    async def _open_learning_trade(self, order, decision, risk_amount: float) -> None:
        """Paper-fill a skipped signal on the private learning broker.

        It never touches a real or shadow account — its only purpose is to close
        later (on the same stop/target) and hand the policy the counterfactual.
        """
        ...
        if self.learn_broker.position(order.symbol, strat_name) is not None:
            return  # already tracking this strategy's counterfactual for the symbol
    ...
    async def _consensus_step(self, symbol: str, features, ctx: dict,
        ...
        # Derive the account execution mode from runtime state. Never infer it
        # from strategy presence: shadow contributors must remain paper-only.
        go_live = not self._paper_mode and not self._account_is_paper()
        pos = self.broker.position(symbol, "consensus")
        ...
        if d.action in ("none", "veto") and d.side is not None and mark > 0:
            ...
            obs = {"ts": datetime.now(timezone.utc).isoformat(), "symbol": symbol,
        ...
        if d.action == "exit":
            ...
            await self._close(self.broker, symbol, ctx, "consensus")
            return
        ...
        trade = await self.broker.submit(order)
        ...
        if trade:
            ...
            self.risk.open_exposure[symbol] = (
                self.risk.open_exposure.get(symbol, 0.0)
                + abs((order.volume or 0.0) * features.last_price))
    ...
    async def _process_symbol(self, symbol: str) -> None:
        ...
        for strat in active_strats:
            ...
            for signal in signals:
                ...
                raw = self.sizer.size(signal, features, self.risk.equity)
                ...
                if self.rl is not None:
                    ...
                    if self._rl_size_scale and not self.rl.warming_up:
                        ...
                        if instrument_min:
                            ...
                            scaled = max(scaled, instrument_min)
                ...
                if existing is not None:
                    if existing.side == signal.side:
                        self.journal.record_signal(
                            signal, mode, features.last_price, sentiment=sent_score,
                ...
                if trade:
                    ...
                    trade.context = {**(trade.context or {}),
                                     ...
                                     "regime": regime}
                    # Track exposure intra-loop so the position/exposure caps bind
                    # *within* a loop, not just at the next loop's refresh. Only the
                    # active account broker consumes the real exposure budget;
                    # shadow/learning fills do not. (Conservative: not decremented on
                    # intra-loop closes — the loop-start refresh corrects any drift.)
